...
import nl.giantit.minecraft.giantcore.database.query.Join;
 ...
 * @author Giant
 */
public class SQLiteSelectQuery implements SelectQuery {
	...
	public SQLiteSelectQuery(Driver db) {
		this.db = db;
	}
	...
	public Join join(String table) {
		return this.join(new SQLiteJoin(this.db, table));
	}
